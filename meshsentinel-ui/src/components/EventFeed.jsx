...
function TrustBadge({ trust }) {
  ...
  return (
    <div style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', padding: '4px 12px', borderRadius: '20px', background: m.bg, border: `1px solid ${m.border}`, color: m.color, fontSize: '11px', fontWeight: 700, letterSpacing: '0.08em', fontFamily: 'var(--mono)' }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <span style={{ fontSize: '9px', fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '0.1em' }}>VERIFICATION PROGRESS</span>
        {nextLabel ? <span style={{ fontSize: '9px', color: 'var(--text-muted)' }}>{cc}/{nextAt} → {nextLabel}</span> : <span style={{ fontSize: '9px', color: 'var(--trust-high)', fontWeight: 700 }}>✓ MAX TRUST</span>}
      </div>
...
// ── NEW: Photo thumbnail with lightbox ──────────────────────────────────────
function AlertPhoto({ imageUrl }) {
  const [lightbox, setLightbox] = useState(false)
  const [errored,  setErrored]  = useState(false)
  if (!imageUrl || errored) return null
  return (
    <>
      <div style={{ borderRadius: '8px', overflow: 'hidden', border: '1px solid var(--border)', background: 'rgba(255,255,255,0.02)', padding: '8px 10px' }}>
        <div style={{ fontSize: '9px', fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '0.1em', marginBottom: '6px' }}>📷 PHOTO EVIDENCE</div>
        <img
          src={imageUrl}
          alt="Alert evidence"
          onError={() => setErrored(true)}
          onClick={() => setLightbox(true)}
          style={{ width: '100%', maxHeight: '200px', objectFit: 'cover', borderRadius: '6px', cursor: 'pointer', display: 'block' }}
        />
        <div style={{ fontSize: '10px', color: 'var(--text-muted)', marginTop: '4px' }}>Click to enlarge</div>
      </div>
      {lightbox && (
        <div onClick={() => setLightbox(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.92)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', cursor: 'pointer' }}>
          <img src={imageUrl} alt="Full size" onClick={e => e.stopPropagation()} style={{ maxWidth: '90vw', maxHeight: '90vh', objectFit: 'contain', borderRadius: '8px', cursor: 'default' }} />
          <button onClick={() => setLightbox(false)} style={{ position: 'absolute', top: '20px', right: '20px', background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '8px', color: '#fff', fontSize: '16px', width: '36px', height: '36px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
        </div>
      )}
    </>
  )
...
function VerificationPanel({ event, myDeviceId, onAction }) {
  ...
  if (isOwn) return (
    <div style={styles.vpRow}><span style={styles.vpIcon}>📡</span><span style={styles.vpMuted}>You broadcast this alert — awaiting peer verifications</span></div>
  )
    ...
    return (
      <div style={{ ...styles.vpRow, background: 'rgba(16,185,129,0.06)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: '10px', padding: '10px 14px' }}>
        ...
        <div>
          ...
          <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '2px' }}>Total cross-checks: {cc} · Trust now: {tr}</div>
        </div>
      </div>
    )
  }
  if (dismissed || (phase === 'done' && result?.action === 'dismiss')) return (
    <div style={{ ...styles.vpRow, ...styles.vpMutedBox }}><span style={{ fontSize: '13px' }}>🚫</span><span style={styles.vpMuted}>You marked this as unconfirmable — no cross-check sent</span></div>
  )
  if (!pendingVerify && phase === 'idle') return (
    <div style={{ ...styles.vpRow, ...styles.vpMutedBox }}>
      <span style={{ fontSize: '13px' }}>👁️</span>
      <div style={{ flex: 1 }}><span style={styles.vpMuted}>Can you witness this emergency? </span><button style={styles.vpInlineBtn} onClick={() => setPhase('confirming')}>Verify manually</button></div>
    </div>
  )
  if (pendingVerify && phase === 'idle') return (
    <div style={styles.vpPendingBox}>
      <div style={styles.vpPendingHeader}>
        ...
        <div>
          ...
          <div style={styles.vpPendingDesc}>This alert arrived from a peer. If you can see or confirm this situation, tap <strong>Yes, I can confirm</strong>. Your device will be broadcast as an independent cross-check, raising trust for everyone in the mesh.</div>
        </div>
      </div>
      <div style={styles.vpBtnRow}>
        <button style={styles.vpDismissBtn} onClick={() => setPhase('dismissing')}>🚫 No / Dismiss</button>
        <button style={styles.vpVerifyBtn}  onClick={() => setPhase('confirming')}>✅ Yes, I can confirm</button>
      </div>
    </div>
  )
  if (phase === 'confirming') {
    const doVerify = async () => {
      ...
      } catch { setResult({ error: 'Could not reach backend', action: 'verify' }); setPhase('error') }
    }
    return (
      <div style={styles.vpConfirmBox}>
        <div style={styles.vpConfirmTitle}>⚠️ Confirm your cross-check</div>
        <div style={styles.vpConfirmDesc}>By confirming, you are stating that <strong>you can personally witness or confirm</strong> this emergency is real. Your device ID will be permanently recorded as a verifier and broadcast to all peers.</div>
        <div style={styles.vpBtnRow}>
  ...
  if (phase === 'dismissing') {
    const doDismiss = async () => {
      ...
      try {
        ...
        setPhase(res.ok ? 'done' : 'error')
        if (res.ok) onAction && onAction()
      } catch { setResult({ error: 'Could not reach backend', action: 'dismiss' }); setPhase('error') }
    }
    return (
      <div style={styles.vpConfirmBox}>
        <div style={styles.vpConfirmTitle}>Dismiss this verification request?</div>
        <div style={styles.vpConfirmDesc}>You won't be asked again for this alert. No cross-check is sent — the alert stays visible in the feed and can still gain trust from other devices.</div>
        <div style={styles.vpBtnRow}>
          <button style={styles.vpCancelBtn} onClick={() => setPhase('idle')}>← Go back</button>
  ...
  if (phase === 'sending') return (
    <div style={{ ...styles.vpRow, ...styles.vpMutedBox }}><span>⏳</span><span style={styles.vpMuted}>Broadcasting your decision to all peers…</span></div>
  )
  if (phase === 'error') return (
    <div style={{ ...styles.vpRow, background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: '10px', padding: '10px 14px' }}>
      <span>❌</span><span style={{ fontSize: '12px', color: 'var(--trust-low)', flex: 1 }}>{result?.error}</span>
...
function EventCard({ event, myDeviceId, onVerificationAction }) {
  ...
  const imageUrl       = event.image_url       || null   // ← NEW
  const originDevice   = event.origin_device   || ''
  ...
  return (
    <div style={{ ...styles.card, borderLeft: `3px solid ${meta.color}`, opacity: fading ? 0.35 : 1, boxShadow: pendingVerify ? `0 0 0 2px rgba(245,158,11,0.35)` : undefined, transition: 'opacity 1s ease, box-shadow 0.3s ease' }}>
      {pendingVerify && <div style={styles.pendingBanner}><span>⚠️</span> This alert needs your verification</div>}
      <div style={styles.cardTop}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flex: 1 }}>
          <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: meta.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px', flexShrink: 0 }}>{meta.icon}</div>
          <div>
            <div style={{ fontSize: '15px', fontWeight: 700, color: meta.color }}>{meta.label} Alert</div>
            <div style={{ fontSize: '11px', color: 'var(--text-muted)', fontFamily: 'var(--mono)' }}>
              ...
              {imageUrl && <span style={{ marginLeft: '6px' }}>📷</span>}
            </div>
          </div>
        </div>
      ...
      {/* ── Photo — only on live alerts page, right after description ── */}
      <AlertPhoto imageUrl={imageUrl} />

      <div style={styles.statsRow}>
        <div style={styles.statBox}><div style={styles.statNum}>{devicesReached}</div><div style={styles.statLabel}>devices reached</div></div>
        <div style={styles.statDivider} />
        <div style={styles.statBox}><div style={{ ...styles.statNum, color: crossChecks >= 2 ? 'var(--trust-medium)' : undefined }}>{crossChecks}</div><div style={styles.statLabel}>cross-checks</div></div>
        <div style={styles.statDivider} />
        <div style={styles.statBox}><div style={styles.statNum}>{maxHop}</div><div style={styles.statLabel}>max hops</div></div>
      </div>

      {!authorizedNode && <TrustProgress crossChecks={crossChecks} trust={trust} />}
      <VerificationPanel event={event} myDeviceId={myDeviceId} onAction={onVerificationAction} />
          ...
          {expanded && reachedIds.length > 0 && (
            <div style={styles.chipGroups}>
              {reachedIds.map(d => {
                ...
                return (
                  <span key={d} style={{ ...styles.deviceChip, ...(isOrigin ? styles.chipOrigin : {}), ...(isVerifier ? styles.chipVerifier : {}) }}>
                    {isOrigin && '📡 '}{isVerifier && !isOrigin && '✓ '}{d.replace('DEVICE-', '')}
                  </span>
                )
              })}
              <div style={styles.chipLegend}><span>📡 origin</span><span>✓ verified</span><span style={{ color: 'var(--text-muted)', fontStyle: 'italic' }}>plain = received only</span></div>
            </div>
          )}
        </div>
      )}

      {!isVerified && !fading && secondsLeft !== null && secondsLeft <= 30 && <div style={styles.ttlCountdown}>⏱ Fades in {secondsLeft}s if not verified</div>}
      {fading && <div style={styles.ttlNotice}>⏳ Unverified alert expired — awaiting confirmation</div>}
    </div>
  )
}
...
export default function EventFeed({ events, myDeviceId, pendingCount, onRefresh, onVerificationAction }) {
  ...
  const filtered = filter === 'ALL' ? sorted : sorted.filter(e => e.type === filter || e.trust === filter)
    ...
    <div style={styles.feed}>
      <div style={styles.toolbar}>
        <div style={styles.toolbarLeft}>
          ...
          {pendingCount > 0 && <span style={styles.pendingPill}>{pendingCount} need{pendingCount === 1 ? 's' : ''} verification</span>}
        </div>
      ...
      <div style={styles.cards}>
        {filtered.length === 0 ? (
          <div style={styles.empty}>
            ...
            <div style={styles.emptyText}>MeshSentinel is listening on the mesh.<br />When a peer broadcasts an alert it will appear here instantly.</div>
          </div>
        ) : filtered.map(e => (
          <EventCard key={e.event_id} event={e} myDeviceId={myDeviceId} onVerificationAction={onVerificationAction} />
        ))}
      </div>
    </div>
  )
