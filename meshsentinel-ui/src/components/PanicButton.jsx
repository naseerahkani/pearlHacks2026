...
export default function PanicButton({ onBroadcast }) {
  ...
  const handleConfirm = async () => {
    ...
    setSending(true)
    await onBroadcast(confirming, authorized, description.trim(), location.trim())
    setSending(false)
    setOpen(false)
    setConfirming(null)
    setAuthorized(false)
              ...
              <>
                ...
                <div style={styles.fieldGroup}>
                  <label style={styles.fieldLabel}>
                    DESCRIPTION <span style={styles.optional}>(optional but helpful)</span>
                  </label>
                  <textarea
                    placeholder="e.g. Smoke coming from 3rd floor stairwell, building evacuating..."
                    value={description}
                ...
                <div style={styles.fieldGroup}>
                  ...
                  <input
                    type="text"
                    placeholder="e.g. Granville East, Room 204, Pittsboro St corner..."
                    value={location}
                ...
                <div style={styles.authorizedRow}>
                  <label style={styles.authLabel}>
                    ...
                    <span>
                      I am an{' '}
                      <strong style={{ color: 'var(--trust-high)' }}>Authorized Node</strong>
                      {' '}— instantly sets trust to HIGH
                    </span>
                  </label>
                </div>
              </>
...
const styles = {
  ...
  panicBtn: {
    ...
    fontFamily: 'var(--font)', fontWeight: 800, letterSpacing: '0.04em',
    boxShadow: '0 0 24px rgba(220,38,38,0.4)', width: '100%', maxWidth: '400px',
  ...
  modalTitle: {
    display: 'flex', alignItems: 'center', gap: '10px',
    fontSize: '17px', fontWeight: 800, color: 'var(--text-primary)', letterSpacing: '-0.02em',
  },
  closeBtn: {
  ...
  typeBtn: {
    display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px',
    padding: '18px 10px', border: '1.5px solid', borderRadius: '14px',
    cursor: 'pointer', fontFamily: 'var(--font)', transition: 'transform 0.15s ease',
  },
  ...
  authLabel: {
    display: 'flex', alignItems: 'flex-start', fontSize: '12px',
    color: 'var(--text-secondary)', cursor: 'pointer', lineHeight: 1.5,
  },
  confirmPane: {
    display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px', paddingTop: '8px',
  },
  ...
  authBadge: {
    background: 'var(--trust-high-bg)', border: '1px solid var(--trust-high-border)',
    color: 'var(--trust-high)', fontSize: '12px', fontWeight: 600,
  ...
  cancelBtn: {
    flex: 1, padding: '11px', border: '1px solid var(--border)', borderRadius: '10px',
  ...
  sendBtn: {
    ...
    fontFamily: 'var(--font)', letterSpacing: '0.02em', transition: 'opacity 0.2s ease',
  },
}